...
def agglomerative_cluster(B_t, M, k):
    ...
    for (x_i, y_i) in B_t:
        ...
        if len(M) < k:
            M.append([x_i, y_i, a_i, 1,i]) # here w=1
            ...
            if np.linalg.norm(a_i - a_prime) <= np.linalg.norm(M[j][2] - a_prime):
                M[j] = [x_i, y_i, a_prime, M[j][3],i]
            M[j][2]=a_prime     # feature update
            M[j][2]+=1           # w update 
...
B_t = [(torch.randn((3, 224, 224)), 0) for _ in range(100)]  # Replace this with your actual data
M = []
k = 50
M = agglomerative_cluster(B_t, M, k)

# To see which occurance of data points are in the buffer
for i in M:
  print(i[4])

...
''''Below parts are included by me'''
#########################################################################################################
###        Split CIFAR-10
#########################################################################################################
def get_split_cifar10(task_id, batch_size, cifar_train, cifar_test):
    ...
    start_class = int((task_id-1)*0.5)
    end_class = int(task_id * 0.5)
...
def get_subset_split_cifar10(task_id, batch_size, cifar_train, num_examples):
    ...
    start_class = (task_id-1)*0.5
    end_class = task_id * 0.5
...
def get_imb_split_cifar10(task_id, batch_size, cifar_train, cifar_test,  limit_per_task = 1000):
    imratio = 0.1
    transform_train = torchvision.transforms.Compose([
        torchvision.transforms.RandomCrop(32, padding=4),
        torchvision.transforms.RandomHorizontalFlip(),
    ...
    max_iter = 20
    n_tasks = max_iter
    perm = np.arange(10)  # changed from 100 to 10
    # np.random.shuffle(perm)
    cpt = int(10 / n_tasks)       # changed from 100 to 10
    cur_iter = 0
    inds = []
    inds_test = []
    rs = np.random.RandomState(0)
...
def get_subset_imbalanced_split_cifar10(task_id, batch_size, cifar_train, num_examples):
    ...
    img_max = len(cifar_train.targets) // n_class
    imb_factor = 1. / 10      # changed from 100 to 10
    trains = []
    for class_number in range(start_class, end_class):
        target = (targets_train == class_number)
        per_class_examples = int(img_max * (imb_factor**(class_number/(n_class - 1))))
...
def get_all_loaders(seed, dataset, num_tasks, bs_inter, bs_intra, num_examples, per_task_rotation=9, is_coreset=True):
    ...
    elif 'cifar' in dataset and '10' in dataset:
    ...
    if is_coreset:
        ...
        for task in range(1, num_tasks+1):
            ...
            if 'cifar' in dataset and '10' in dataset:
                if 'imb' in dataset:
                    seq_loader_train , seq_loader_val = fast_cifar_loader(get_imbalanced_split_cifar10(task, bs_intra, cifar_train, cifar_test), task, 'cpu')
                    sub_loader_train , _ = fast_cifar_loader(get_subset_imbalanced_split_cifar10(task, bs_inter, cifar_train, 5*num_examples), task, 'cpu')                    
